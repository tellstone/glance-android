...
class FiatPriceClientTest {
    ...
    @Test
    fun `mempool accepts array history response for one timestamp`() {
        MockWebServer().use { server ->
            server.start()
            server.enqueue(MockResponse().setBody("""{"prices":[{"time":100,"USD":123.45}]}"""))
            val client = MempoolFiatPriceClient(server.url("api/").toString(), DirectNetworkClientFactorySource)

            assertEquals(
                listOf(FiatQuote(FiatProvider.MEMPOOL_SPACE, "USD", 123.45, 100L)),
