...
class FiatPriceClientTest {
    ...
    fun `mempool parses a current price without credentials`() {
        MockWebServer().use { server ->
            server.start()
            server.enqueue(MockResponse(body = "{\"USD\":123.45}"))
            ...
            assertEquals("/api/v1/prices", server.takeRequest().url.encodedPath)
        }
    }
    ...
    fun `mempool full history is fetched in one request and filters the requested range`() {
        MockWebServer().use { server ->
            server.start()
            server.enqueue(MockResponse(body = """{"prices":[{"time":100,"USD":10.0},{"time":200,"USD":20.0},{"time":300,"USD":-1.0}]}"""))
            ...
            assertEquals("/api/v1/historical-price", request.url.encodedPath)
    ...
    fun `mempool uses the timestamp endpoint for one missing bucket`() {
        MockWebServer().use { server ->
            server.start()
            server.enqueue(MockResponse(body = """{"prices":{"USD":123.45}}"""))
            val client = MempoolFiatPriceClient(server.url("api/").toString(), DirectNetworkClientFactorySource)

            assertEquals(listOf(FiatQuote(FiatProvider.MEMPOOL_SPACE, "USD", 123.45, 100L)), client.historicalPrices("USD", 100L, 100L))

            val request = server.takeRequest()
            assertEquals("USD", request.url.queryParameter("currency"))
            assertEquals("100", request.url.queryParameter("timestamp"))
        }
    }
    ...
    fun `mempool rejects a zero price from the timestamp endpoint`() {
        MockWebServer().use { server ->
            server.start()
            server.enqueue(MockResponse(body = """{"prices":{"USD":0}}"""))
            val client = MempoolFiatPriceClient(server.url("api/").toString(), DirectNetworkClientFactorySource)
